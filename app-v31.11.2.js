...
window.__floewAppVersion="31.11.2";
...
const ADS_CACHE_KEY="thefloew.adsCatalog.v2";
...
async function loadAdsCatalog(){
  try{
    ...
    if(!response.ok){
      throw new Error(`GitHub ads HTTP ${response.status}`);
    }

    const data=await response.json();

    /*
      Contents API bir klasör için normalde doğrudan dizi döndürür.
      object media type kullanan olası cevapları da entries ile destekliyoruz.
    */
    ...
    /*
      GitHub API geçici olarak ulaşılamaz veya rate-limit'e girerse mevcut
      çalışan katalog silinmez. İlk açılışsa son başarılı katalog kullanılır.
    */
    console.warn("GitHub ads catalog:",err);
