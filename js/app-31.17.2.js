...
window.__floewAppVersion="31.17.2";
...
function switchFeedMode(nextMode){
  ...
  */
  ...
  /*
    Sekmeler soldan sağa "Son dakika | Gündem".
    Gündem -> Son dakika: yeni içerik soldan gelir.
    Son dakika -> Gündem: yeni içerik sağdan gelir.
  */
  const toBreaking=next==="breaking";
  const enterClass=toBreaking ? "enter-right" : "enter-left";
  const exitClass=toBreaking ? "exit-right" : "exit-left";
      ...
      ? rows.map(row=>{
          ...
          return `
            <div class="public-stat-category">
              <span>${publicStatsEscape(row.category||"Bilinmiyor")}</span>
              <i><b style="width:${Math.max(2,value/max*100)}%"></b></i>
              <small>${publicStatsNumber(value)}</small>
            </div>
          `;
        }).join("")
      : `<div class="public-stats-empty">Henüz kategori verisi yok.</div>`;
  }
...
async function loadPublicStats(range=publicStatsRange){
  ...
  try{
    const response=await fetch(
      `${PUBLIC_STATS_API}?range=${encodeURIComponent(range)}`,
      {cache:"no-store"}
    );
    const data=await response.json();
...
window.addEventListener("pointerup",e=>{
  ...
  /*
    Yatay swipe = akış sekmesi.
      Sağa: Son dakika (soldaki sekme)
      Sola: Gündem (sağdaki sekme)

    Dikey swipe = haber navigasyonu.
  */
  if(
    Math.abs(dx)>=SWIPE &&
    ...
    switchFeedMode(dx>0 ? "breaking" : "agenda");
  }else if(
    Math.abs(dy)>=SWIPE &&
