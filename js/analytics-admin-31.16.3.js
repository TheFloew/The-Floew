...
function databaseStatusMessage(ping){
  ...
  return {
    state:"ready",
    text:"✓ CONTENT_DB, AUDIENCE_DB ve BEHAVIOR_DB hazır. Analytics veri toplamaya hazır."
  };
}
...
async function authenticate(){
  ...
  try{
    ...
    const info=databaseStatusMessage(ping);
    setDatabaseStatus(`V31.16.3 · ${info.text}`,info.state);
...
async function setup(){
  ...
  console.info("[Flöw Analytics] setup clicked", new Date().toISOString());
  button.disabled=true;
  button.textContent="Hazırlanıyor...";
  setDatabaseStatus(
    "D1 tabloları oluşturuluyor. Bu işlem birkaç saniye sürebilir...",
    "working"
  );
  status.textContent="";
  ...
  try{
    ...
    setDatabaseStatus(
      "✓ Üç D1 veritabanının tabloları ve index'leri hazırlandı.",
      "ready"
    );
    status.textContent="Analytics hazır. İlk event'ler geldikçe istatistikler dolmaya başlayacak.";
    await loadDashboard();
    ...
    status.textContent=
      "Kurulum tamamlanamadı. Yukarıdaki hata mesajını kontrol edin.";
  }finally{
