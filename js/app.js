...
window.__floewAppVersion="31.75.0";
...
const VIDEO_ONLY_TARGET_COUNT=48;
const VIDEO_ONLY_MIN_PER_SOURCE=2;
const VIDEO_ONLY_SOURCE_SAMPLE_MAX=8;
...
function currentVideoOnlyVerdict(story){
  /*
    News Worker v31.90 doğrudan video RSS/discovery sonuçlarında güvenilir
    haber-video bağını işaretler. Bu haberleri yeniden /video ile taramadan
    videolu havuza al; böylece video RSS kaynakları küçük tarama batch'lerinde
    aç kalmaz.
  */
  if(story?.videoVerified===true)return true;
...
function videoOnlyLikelyScore(story){
  ...
  if(story?.videoVerified)score+=320;
  if(story?.videoDiscovery)score+=220;
  if(story?.videoArticleHint)score+=180;
  if(story?.video)score+=120;
...
function applyFilters(options={}){
  ...
  /*
    Video-only taraması yeni verdict geldikçe applyFilters() çağırır. Eski
    davranış her çağrıda history'yi [idx] olarak sıfırladığı için kullanıcı
    birkaç haber ilerledikten sonra geri dönemiyordu. preserveScan çağrılarında
    geçmişi haber kimliğiyle yeni filtrelenmiş listeye yeniden eşle.
  */
  const preserveHistory=Boolean(
    options?.preserveScan &&
    state.stories.length
  );
  const oldStories=preserveHistory
    ? state.stories
    : null;
  const oldHistory=preserveHistory && Array.isArray(state.history)
    ? [...state.history]
    : [];
  const oldHistoryPos=preserveHistory && Number.isInteger(state.historyPos)
    ? state.historyPos
  ...
  if(preserveHistory){
    ...
    oldHistory.forEach((oldIndex,position)=>{
      ...
      if(mappedHistory[mappedHistory.length-1]!==mapped){
        mappedHistory.push(mapped);
      }

      if(position<=oldHistoryPos){
        mappedPos=mappedHistory.length-1;
      }
    });

    if(!mappedHistory.length){
      mappedHistory.push(idx);
      mappedPos=0;
    }else{
      mappedPos=Math.max(
        0,
        Math.min(mappedPos,mappedHistory.length-1)
      );

      if(mappedHistory[mappedPos]!==idx){
        mappedHistory.splice(mappedPos+1);
        if(mappedHistory[mappedHistory.length-1]!==idx){
          mappedHistory.push(idx);
        }
        mappedPos=mappedHistory.length-1;
      }
    ...
    state.history=mappedHistory;
    state.historyPos=mappedPos;
  }else{
    state.history=[idx];
    state.historyPos=0;
    skippedAdHistory=null;
...
async function resolveStoryMedia(story,options={}){
  ...
  const strictVideoOnly=Boolean(options?.strictVideoOnly);
  if((!videoEnabled && !force) || !story)return null;

  const baseKey=mediaKey(story);
  if(!baseKey)return null;

  /*
    Normal oynatma ile "Sadece videolu haberler" doğrulaması aynı cache
    sonucunu paylaşamaz. Normal resolver sayfadaki zayıf/generic bir player'ı
    oynatabilir; strict tarama ise yalnız habere bağlı videoyu kabul eder.
  */
  const key=strictVideoOnly
    ? `${baseKey}|strict-video-only`
    : baseKey;
    ...
    */
    ...
    if(current){
      if(current.videoVerified===true){
        ...
        /*
          Normal playback cache'i video-only doğrulamasında kullanma. Yalnız
          strict resolver sonucuyla işaretlenmiş medya bu modu doğrulayabilir.
        */
        const strictKey=
          `${mediaKey(current)}|strict-video-only`;
        const cached=
          storyMediaCache.get(strictKey);
    ...
    function mergeDuplicateStory(existing,candidate){
      ...
      return {
        ...
        videoDiscoveryProvider:
          preferred.videoDiscoveryProvider ||
          other.videoDiscoveryProvider ||
          ""
      };
    }
